# WorkTrackr — CRM, Ordering & Commission Roadmap (v2.7)
...
**Build position:** Phases 0–8 + Phase 8.1 (v2.1) + **v2.5 role selector/role-based home** + **v2.6 engineer profit lockdown** are built and in the deployed bundle (verify they render after a hard-refresh). No phase in progress. **Top of the queue: a billing + subscription-enforcement batch** (see §14.26) — enforcement is currently a no-op and "Manage Billing" fails for orgs with no Stripe customer. Operational caution: do NOT run `vite build` in the Render shell (512 MB → OOM → outage); redeploy via Render to rebuild.
