...
**Build position:** Phases 0–8 are built and deployed. The redesign is essentially complete end-to-end. No phase is in progress; only the deferred non-blockers below remain.
...
- **Sales** — a single **Sales** entry that opens a tabbed page: **Companies · Leads · Quotes · Orders · Contracts** (tabs via `SalesTabs.jsx`; clicking a row still opens the full company profile).
- **Finance** — Invoices.
