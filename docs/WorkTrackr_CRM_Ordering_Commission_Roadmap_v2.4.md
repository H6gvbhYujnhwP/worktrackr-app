# WorkTrackr — CRM, Ordering & Commission Roadmap (v2.4)
...
- **`memberships.role` CHECK widening** — ✅ done in v2.4 (`'salesman'`/`'engineer'` now allowed; migration `phase9_widen_membership_roles.sql` + `database/schema.sql`). The *remaining* build on top: a Users role selector + role-based home routing (Salesman → My commission, Engineer → My wage). Manager-gating still works off `admin`/`manager`/`owner`/`partner_admin`.
