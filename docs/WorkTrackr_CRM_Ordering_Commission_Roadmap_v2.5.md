# WorkTrackr — CRM, Ordering & Commission Roadmap (v2.5)
...
**Build position:** Phases 0–8 plus the **Phase 8.1 Sales UX consolidation (v2.1)** are built, plus the **v2.5 Users role selector + role-based home screens**. The redesign is complete end-to-end. No phase is in progress; only the deferred non-blockers below remain (notably full per-role sidebar/route gating).
...
- **`memberships.role` CHECK widening** — ✅ done in v2.4. The Users **role selector + role-based home routing** is now ✅ built in v2.5 (Salesman → My Pay/Commission, Engineer → My Pay/Wage; My Pay hides Commission from engineers). **Remaining:** full per-role **sidebar/route gating** (so engineers can't navigate to Sales/profit at all) — recommended as the next batch. Manager-gating still works off `admin`/`manager`/`owner`/`partner_admin`.
...
**Stale docs — do NOT trust:** `docs/ROADMAP.md`, `docs/APP-STATE.md` and `docs/database_schema.md` are pre-redesign (April). Trust **this roadmap (v2.5)** and the **code** over them.
