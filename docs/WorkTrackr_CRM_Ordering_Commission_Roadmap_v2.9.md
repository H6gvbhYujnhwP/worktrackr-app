# WorkTrackr — CRM, Ordering & Commission Roadmap (v2.9)
...
**Build position:** Phases 0–8 + Phase 8.1 (v2.1) + **v2.5 role selector/role-based home** + **v2.6 engineer profit lockdown** are built. **v2.8 billing + subscription enforcement is BUILT and handed over (12 files), but NOT yet deployed or confirmed live by the owner.** Nothing from v2.8 is in this zip's source unless the owner has dropped the handover files in — treat the zip as pre-v2.8 unless verified.
