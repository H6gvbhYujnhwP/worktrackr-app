# WorkTrackr — CRM, Ordering & Commission Roadmap (v2.8)
...
**Build position:** Phases 0–8 + Phase 8.1 (v2.1) + **v2.5 role selector/role-based home** + **v2.6 engineer profit lockdown** + **v2.8 billing + subscription enforcement** are built. The subscription wall is now real and enforced inside `authenticateToken` (blocks expired trial / cancelled / failed-payment orgs with a 402; a failed payment blocks immediately). **Owner dev org is exempt** (`billing_exempt=true` + a 100-year trial) so enforcement cannot lock it out. **Two same-class property bugs remain as the next small batch:** `email-intake.js` settings endpoints 401 (wrong org-id property + no auth on mount) and `quotes-from-ticket.js` runs with no org (reads org id off the JWT, which doesn't carry it). Operational caution unchanged: do NOT run `vite build` in the Render shell (512 MB → OOM → outage); redeploy via Render to rebuild. **Before deploying v2.8: set `ADMIN_API_KEY` in Render** or the admin back-door (set-trial/update-plan) stops working.
...
**Stale docs — do NOT trust:** `docs/ROADMAP.md`, `docs/APP-STATE.md` and `docs/database_schema.md` are pre-redesign (April). Trust **this roadmap (v2.7)** and the **code** over them.
