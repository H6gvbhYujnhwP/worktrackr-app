...
## Source of truth
- **The roadmap** — `docs/WorkTrackr_CRM_Ordering_Commission_Roadmap_v3.9.md` (always read the highest version). Start with **§15 (status banner)** and **§17 (UI redesign + build log)**.
...
## Next task
**Continue Delivery (`batch_b`).** Two pieces remain, each its own turn:
1. **Calendar (`CRMCalendar.jsx`, ~1,400 lines).** Do it as a **surgical dark pass** (copy + scripted colour-token swap, logic untouched — same method used for `JobDetail`), since it has 11 live flows (events CRUD, AI suggestions, source toggles). **Finishing it also lets the Sales `Calendar` tab go dark** — once done, set `SalesTabs dark` for `sales-calendar` too and add `sales-calendar`/`calendar` to the Dashboard full-bleed list. Check `@/components/ui/select.jsx` (shadcn) renders OK on dark.
2. **Tickets** — the biggest, most critical screen: the list is **woven into `Dashboard.jsx`** (inline stat cards + tabs + table) on top of `TicketsTableView.jsx` (254) and `TicketDetailViewTabbed.jsx` (~1,900), plus `CreateTicketModal`, `TicketDetailModal`, `AssignTicketsModal`, `TicketFieldCustomizer`. Inventory every flow first; reskin in place; **do not** cram it with anything else.

After Delivery: **Finance** (batch_d: Invoices), **Settings** (batch_e), **Account** (batch_f), plus the **sidebar 3-state + mobile**. **Keep Manus's design — don't redesign.** Per the standing rules: inventory each old screen first, **verify each flow works against a mounted endpoint**, reskin-in-place where only colours change (keep logic verbatim), build, validate, hand over with a filename→folder + DELETE list, bump the roadmap +0.1.
