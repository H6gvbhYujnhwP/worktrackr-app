...
## Source of truth
- **The roadmap** — `docs/WorkTrackr_CRM_Ordering_Commission_Roadmap_v3.8.md` (always read the highest version). Start with **§15 (status banner)** and **§17 (UI redesign + build log)**.
...
## Next task
**Delivery group (`batch_b`): Tickets (list + detail), Projects (list + detail), Calendar.** Doing the **Calendar** dark pass here also lets the Sales **Calendar** tab's chrome go dark (it's been left light on purpose until CRMCalendar is redesigned). After Delivery: **Finance** (batch_d: Invoices), **Settings** (batch_e: Users, Commission rules, Engineer wages, CRM settings, Catalogue, Pricing, Billing, Security, Email Intake), **Account** (batch_f: Login, Subscription paywall), plus the **sidebar 3-state + mobile**. **Keep Manus's design — don't redesign.** Per the standing rules: inventory each old screen's features first, **verify each flow actually works against a mounted endpoint** (some flows are known broken — fix or flag), reskin-in-place where only colours change, build, validate, and hand over with a filename→folder + DELETE list, bumping the roadmap +0.1.
