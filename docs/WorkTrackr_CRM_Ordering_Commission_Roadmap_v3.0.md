# WorkTrackr — CRM, Ordering & Commission Roadmap (v3.0)
...
**🎨 BIGGEST CURRENT WORKSTREAM — full-app UI REDESIGN (design done, build NOT started).** See the new **§17** and **`docs/design/REDESIGN_HANDOVER.md`** for the full picture. In short: the whole app is being redesigned to a **fully dark, full-width** look (Concept 3 "Relationship Hub"), designed by **Manus AI** and signed off by the owner. The **company is the hub** (Leads tab removed → folded into the company; Contracts tab removed → services shown on the company). Sidebar becomes a slim icon rail (expandable + hideable; bottom bar on mobile). All design assets + the token sheet live in `docs/design/`. **No redesign code exists yet — design only.** Build to the Manus spec exactly, in small batches, only on the owner's go-ahead.
...
*Status: redesign DESIGN complete; BUILD not started. Roadmap is now v3.0.*
