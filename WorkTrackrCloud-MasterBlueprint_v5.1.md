...
- **`POST /api/admin/update-plan`**: Manually updates an organization's subscription plan. This is a temporary admin tool and should be replaced with a proper admin interface.
  - **Requires:** `ADMIN_KEY` environment variable in the request header.
  - **Body:** `{ "organisationId": "<uuid>", "plan": "<pro|enterprise>" }`

### 10.2. Database Migrations

- **`POST /api/migrations/run`**: Runs all pending database migrations. This is a temporary admin tool.
  - **Requires:** `ADMIN_KEY` environment variable in the request header.
...
| `ADMIN_KEY` | Secret key for accessing admin routes | `your-admin-secret-key` |
...
2.  Install dependencies: `npm install` in both `web` and `frontend` directories.
3.  Create a `.env` file in the `web` directory with the required variables.
4.  Run the backend: `npm run dev` in the `web` directory.
...
### 12.1. Production Deployment (Render)

- **CI/CD:** The project is configured for continuous deployment on Render. Pushing to the `main` branch will automatically trigger a new deployment.
- **Services:**
  - **Web Service (Node.js):** Runs the backend Express server.
  - **Static Site (React):** Serves the frontend React application.
  - **PostgreSQL Database:** Managed PostgreSQL instance.
- **Environment Variables:** All required environment variables must be set in the Render dashboard.

### 12.2. Rollback Procedures

- **Render:** Use the Render dashboard to roll back to a previous deployment.
- **Database:** Database rollbacks are not automatic and require manual intervention. Take database backups before running major migrations.
