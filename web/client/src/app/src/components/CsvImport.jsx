  ...
  return (
    <div className="p-4 md:p-6 max-w-3xl mx-auto">
      ...
      <div className="bg-[#242438] border border-[#2e2e4a] rounded-xl p-4 md:px-5">
        ...
        {step === 1 && (
          <div>
            <label className="flex flex-col items-center justify-center gap-2 border-2 border-dashed border-[#2e2e4a] rounded-xl py-10 cursor-pointer hover:bg-[#1f1f33]">
              <Upload className="w-6 h-6 text-[#6b7280]" />
              <span className="text-[13px] text-[#94a3b8]">Choose a CSV file</span>
              <input type="file" accept=".csv,text/csv" onChange={onFile} className="hidden" />
            </label>
            <div className="text-[12px] text-[#6b7280] mt-3">Or paste rows (first line = column headers):</div>
            <textarea value={raw} onChange={(e) => setRaw(e.target.value)} rows={4} placeholder="Company name,Email,Phone&#10;Acme Ltd,hi@acme.co,01234..." className="w-full border border-[#2e2e4a] rounded-lg px-3 py-2 text-[12.5px] font-mono mt-1" />
        ...
        {step === 2 && (
          <div>
            <div className="text-[13px] text-[#94a3b8] mb-3">{dataRows.length} {dataRows.length === 1 ? 'row' : 'rows'} found. Match your columns to the fields:</div>
            <div className="grid gap-2">
              {FIELDS.map((f) => (
                <div key={f.key} className="grid grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)] items-center gap-2">
                  <div className="text-[13px] text-[#cbd5e1]">{f.label}{f.required && <span className="text-red-600"> *</span>}</div>
                  <select value={mapping[f.key] ?? ''} onChange={(e) => setMapping((m) => ({ ...m, [f.key]: e.target.value }))}
                    className="border border-[#2e2e4a] rounded-lg px-3 py-2 text-[13px] bg-[#242438]">
            ...
            <div className="text-[12px] text-[#94a3b8] mt-3">{mapped.length} {mapped.length === 1 ? 'company' : 'companies'} ready to import. Duplicates (same name or email) are skipped automatically.</div>
            {error && <div className="text-[12px] text-red-700 mt-2">{error}</div>}
            <div className="flex justify-between mt-4">
              <button onClick={() => setStep(1)} className="text-[13px] text-[#94a3b8] hover:text-white">Back</button>
