...
const STATUS_BADGE = {
  ...
  [CONTACT_STATUS.AT_RISK]:  'bg-[#fef3c7] text-[#fcd34d]',
  [CONTACT_STATUS.INACTIVE]: 'bg-[#1f1f33] text-[#94a3b8]',
...
const SECTION_H  = 'text-[11px] font-semibold text-[#6b7280] uppercase tracking-wider mb-2';
...
const GHOST_BTN  = 'inline-flex items-center justify-center w-8 h-8 rounded-lg text-[#6b7280] hover:bg-[#2a2a48] hover:text-[#cbd5e1] transition-colors';

// ── Tab nav component ─────────────────────────────────────────────────────────
function FormTabs({ tabs, active, onChange }) {
  return (
    <div className="flex border-b border-[#2e2e4a] overflow-x-auto">
      {tabs.map(t => (
        <button
          key={t.value}
          type="button"
...
const ContactManager = () => {
  ...
  const getStatusBadge = (status) => STATUS_BADGE[status] || 'bg-[#1f1f33] text-[#94a3b8]';
    ...
    if (activeTab === 'addresses') return (
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <span className="text-[13px] font-semibold text-white">Addresses</span>
          <button type="button" onClick={addAddress} className={GOLD_BTN}><Plus className="w-4 h-4" /> Add Address</button>
        </div>
        {formData.addresses.length === 0 && (
          <p className="text-[13px] text-[#6b7280] text-center py-6">No addresses added yet.</p>
        )}
        {formData.addresses.map((address, index) => (
          <div key={index} className="border border-[#2e2e4a] rounded-lg p-4">
            <div className="flex items-center justify-between mb-4">
              <Select value={address.type} onValueChange={(v) => updateAddress(index, 'type', v)}>
                <SelectTrigger className="w-36 focus:ring-[#f59e0b]/30 focus:border-[#f59e0b]"><SelectValue /></SelectTrigger>
                <SelectContent>
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 bg-[#242438] rounded-xl border border-[#2e2e4a] overflow-hidden">
          {/* Toolbar */}
          <div className="px-5 py-4 border-b border-[#2e2e4a] flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <h2 className="text-[15px] font-semibold text-white">
              Contacts <span className="text-[#6b7280] font-normal text-[14px]">({filteredContacts.length})</span>
            </h2>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="relative">
                ...
                <input
                  className="pl-9 pr-3 py-2 text-[13px] rounded-lg border border-[#2e2e4a] bg-[#1a1a2e] focus:outline-none focus:ring-2 focus:ring-[#f59e0b]/30 focus:border-[#f59e0b] w-48 transition-colors"
              ...
              <Select value={statusFilter} onValueChange={setStatusFilter}>
                <SelectTrigger className="w-36 text-[13px] h-9 focus:ring-[#f59e0b]/30 focus:border-[#f59e0b]">
          ...
          {filteredContacts.length === 0 ? (
            <div className="py-16 text-center">
              ...
              <p className="text-[15px] font-medium text-[#cbd5e1] mb-1">No contacts found</p>
                ...
                <thead>
                  <tr className="bg-[#1f1f33] border-b border-[#2e2e4a]">
                    {['Company', 'Status', 'Contact Info', 'Value', ''].map((h, i) => (
                      <th key={i} className={`px-5 py-2.5 text-[11px] font-semibold text-[#6b7280] uppercase tracking-wider ${i === 4 ? 'text-right' : 'text-left'}`}>
                        {h}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {filteredContacts.map((contact, index) => (
                    <tr
                      ...
                      className={`border-b border-[#2e2e4a] cursor-pointer transition-colors ${
                        selectedContact?.id === contact.id
                          ? 'bg-[rgba(245,158,11,0.08)]'
                          : index % 2 === 1 ? 'bg-[#1f1f33] hover:bg-[rgba(245,158,11,0.08)]' : 'bg-[#242438] hover:bg-[rgba(245,158,11,0.08)]'
                      }`}
                    >
                      {/* Company */}
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-2.5">
                          <div className="w-7 h-7 rounded-lg bg-[#1f1f33] flex items-center justify-center flex-shrink-0">
                            {contact.type === CONTACT_TYPES.COMPANY
                              ? <Building2 className="w-4 h-4 text-[#94a3b8]" />
                              : <User      className="w-4 h-4 text-[#94a3b8]" />}
                          </div>
                          <div>
                            <p className="text-[13px] font-medium text-white">{contact.name}</p>
                            {contact.primaryContact && <p className="text-[11px] text-[#6b7280]">{contact.primaryContact}</p>}
                          </div>
                        </div>
                      </td>
                      {/* Status */}
                      ...
                      <td className="px-5 py-3 text-right">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <button type="button" onClick={(e) => { e.stopPropagation(); handleDeleteContact(contact.id); }} className="inline-flex items-center justify-center w-8 h-8 rounded-lg text-[#6b7280] hover:bg-[#fee2e2] hover:text-red-600 transition-colors">
          ...
          {/* Footer */}
          <div className="px-5 py-2.5 bg-[#1f1f33] border-t border-[#2e2e4a] text-[12px] text-[#6b7280]">
            ...
            <div className="bg-[#242438] rounded-xl border border-[#2e2e4a] overflow-hidden">
              ...
              <div className="px-5 py-4 border-b border-[#2e2e4a] flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className="w-7 h-7 rounded-lg bg-[#1f1f33] flex items-center justify-center">
                    {selectedContact.type === CONTACT_TYPES.COMPANY
                      ? <Building2 className="w-4 h-4 text-[#94a3b8]" />
                      : <User      className="w-4 h-4 text-[#94a3b8]" />}
                  </div>
                  <span className="text-[15px] font-semibold text-white">Contact Details</span>
                </div>
              ...
              <div className="p-5 space-y-5 overflow-y-auto max-h-[calc(100vh-280px)]">
                {/* Name + status */}
                <div>
                  <h4 className="text-[16px] font-semibold text-white mb-2">{selectedContact.name}</h4>
                ...
                <div className="space-y-1.5">
                  ...
                  {selectedContact.primaryContact && <div className="text-[13px] text-[#cbd5e1]"><span className="text-[#6b7280]">Contact: </span>{selectedContact.primaryContact}</div>}
                  {selectedContact.email && <div className="flex items-center gap-2 text-[13px] text-[#cbd5e1]"><Mail className="w-3.5 h-3.5 text-[#6b7280]" />{selectedContact.email}</div>}
                  {selectedContact.phone && <div className="flex items-center gap-2 text-[13px] text-[#cbd5e1]"><Phone className="w-3.5 h-3.5 text-[#6b7280]" />{selectedContact.phone}</div>}
                  {selectedContact.website && <div className="text-[13px] text-[#cbd5e1]"><span className="text-[#6b7280]">Web: </span>{selectedContact.website}</div>}
                </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {selectedContact.addresses.map((address, index) => (
                        <div key={index} className="text-[12px] p-3 bg-[#1f1f33] rounded-lg border border-[#2e2e4a]">
                          <div className="flex items-center justify-between mb-1">
                            <span className="text-[11px] font-semibold text-[#94a3b8] uppercase tracking-wider">{address.type}</span>
                            {address.isPrimary && <span className="text-[11px] font-semibold text-[#f59e0b]">Primary</span>}
                          </div>
                          <p className="text-[#cbd5e1]">{address.line1}</p>
                          {address.line2 && <p className="text-[#cbd5e1]">{address.line2}</p>}
                          <p className="text-[#cbd5e1]">{address.city}, {address.state} {address.postcode}</p>
                          <p className="text-[#cbd5e1]">{address.country}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div>
                  ...
                  <div className="space-y-1.5 text-[13px]">
                    <div className="flex justify-between"><span className="text-[#6b7280]">Total Profit</span><span className="font-semibold text-[#059669]">{formatCurrency(selectedContact.crm.totalProfit, selectedContact.accounting.currency)}</span></div>
                    <div className="flex justify-between"><span className="text-[#6b7280]">Renewals</span><span className="text-[#cbd5e1]">{selectedContact.crm.renewalsCount}</span></div>
                    <div className="flex justify-between"><span className="text-[#6b7280]">Open Opportunities</span><span className="text-[#cbd5e1]">{selectedContact.crm.openOppsCount}</span></div>
                    {selectedContact.crm.industry && <div className="flex justify-between"><span className="text-[#6b7280]">Industry</span><span className="text-[#cbd5e1]">{selectedContact.crm.industry}</span></div>}
                    {selectedContact.crm.source   && <div className="flex justify-between"><span className="text-[#6b7280]">Source</span><span className="text-[#cbd5e1]">{selectedContact.crm.source}</span></div>}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-1.5 text-[13px]">
                    <div className="flex justify-between"><span className="text-[#6b7280]">Currency</span><span className="text-[#cbd5e1]">{selectedContact.accounting.currency}</span></div>
                    <div className="flex justify-between"><span className="text-[#6b7280]">Payment Terms</span><span className="text-[#cbd5e1]">{selectedContact.accounting.paymentTerms}</span></div>
                    {selectedContact.accounting.taxNumber   && <div className="flex justify-between"><span className="text-[#6b7280]">Tax Number</span><span className="text-[#cbd5e1]">{selectedContact.accounting.taxNumber}</span></div>}
                    {selectedContact.accounting.accountCode && <div className="flex justify-between"><span className="text-[#6b7280]">Account Code</span><span className="text-[#cbd5e1]">{selectedContact.accounting.accountCode}</span></div>}
                  </div>
                </div>
                ...
                {selectedContact.notes && (
                  <div>
                    ...
                    <p className="text-[13px] text-[#cbd5e1] p-3 bg-[#1f1f33] rounded-lg border border-[#2e2e4a]">{selectedContact.notes}</p>
                  </div>
                )}
              </div>
            </div>
          ) : (
            <div className="bg-[#242438] rounded-xl border border-[#2e2e4a] py-16 text-center">
              <Users className="w-12 h-12 mx-auto text-[#d1d5db] mb-3" />
              <p className="text-[15px] font-medium text-[#cbd5e1] mb-1">Select a Contact</p>
              <p className="text-[13px] text-[#6b7280]">Choose a contact from the list to view details</p>
            </div>
          )}
        </div>
      </div>
      ...
      {showCreateForm && (
        <div className="fixed inset-0 bg-black/50 flex items-start justify-center z-50 p-4 pt-10 overflow-y-auto">
          <div className="bg-[#242438] rounded-xl border border-[#2e2e4a] w-full max-w-3xl flex flex-col" style={{ maxHeight: '85vh' }}>
            <div className="px-6 py-4 border-b border-[#2e2e4a] flex items-center justify-between flex-shrink-0">
              <h3 className="text-[16px] font-semibold text-white">Create New Contact</h3>
            ...
            <div className="px-6 py-4 border-t border-[#2e2e4a] flex justify-end gap-3 flex-shrink-0">
      ...
      {showEditForm && (
        <div className="fixed inset-0 bg-black/50 flex items-start justify-center z-50 p-4 pt-10 overflow-y-auto">
          <div className="bg-[#242438] rounded-xl border border-[#2e2e4a] w-full max-w-3xl flex flex-col" style={{ maxHeight: '85vh' }}>
            <div className="px-6 py-4 border-b border-[#2e2e4a] flex items-center justify-between flex-shrink-0">
              <h3 className="text-[16px] font-semibold text-white">Edit Contact</h3>
